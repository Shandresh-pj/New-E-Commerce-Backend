...
// ─── Entity Registry ──────────────────────────────────────────────────────────
// All entities are centrally declared in global.ts.
// Importing here ensures TypeORM knows about every table on startup.
import { ALL_ENTITIES } from "../global";

dotenv.config();

const isProduction = process.env.NODE_ENV === "production";

// DB_SYNC=true  → TypeORM auto-syncs schema each restart (safe for dev).
